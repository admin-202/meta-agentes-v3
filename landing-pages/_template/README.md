# Acme LP template (Next.js static export → Cloudflare Pages)

Canonical landing-page template. Cloned per LP by
`.claude/skills/create-landing-page-cliente-exemplo` into `landing-pages/<nome>/`.
...
```bash
npx wrangler pages project create lp-<nome> --production-branch=main
npx wrangler pages deploy out --project-name=lp-<nome> --branch=main
# domain bind (auto-creates proxied CNAME — same Cloudflare account as example.com):
curl -X POST "https://api.cloudflare.com/client/v4/accounts/${CLOUDFLARE_ACCOUNT_ID}/pages/projects/lp-<nome>/domains" \
  -H "Authorization: Bearer ${CLOUDFLARE_API_TOKEN}" -H "Content-Type: application/json" \
  --data '{"name":"<nome>.example.com"}'
```
...
(`localStorage["lp_consent_v1"]`) — never hardcode pixels in `layout.tsx`.
