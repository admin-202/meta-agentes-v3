...
// A ContentDoc equivalent to the seed `curso-exemplo` landing page. The serializer must turn it
...
function ccaDoc(): ContentDoc {
  return {
    settings: {
      ...
      name: "CURSO",
      product: "Curso Exemplo",
      site_url: "https://curso-exemplo.example.com",
      seo: {
        title: "Curso Exemplo — engenharia com IA na prática",
        description:
          "O curso para devs que querem construir software de verdade com Claude Code: arquitetura, agentes e automação.",
        ogAlt: "Curso Exemplo",
      },
      tracking: { fb_pixel_id: "123456789012345", ga4_id: "G-XXXXXXXXXX", consent_key: "lp_consent_v1" },
      checkout_url: "https://pay.example.com/curso-exemplo",
    ...
    sections: [
      ...
      section("comparison", 4, { heading: "Tutorial não leva a produção", ours: "CURSO", theirs: "Genéricos", rows: [{ label: "Método", ours: true, theirs: false }] }),
      ...
      section("authority", 12, { eyebrow: "Instrutor", name: "Cliente Exemplo", bio: "Engenheiro.", credentials: ["10+ anos"] }),
      section("offer", 13, { heading: "Entre para o CURSO", priceLabel: "R$ 1.497", anchor: "De R$ 1.997", bonuses: ["Comunidade"], ctaLabel: "Garantir vaga" }),
      ...
      section("footer", 17, { legal: "© Acme.", links: [{ label: "Termos", href: "#" }] }),
    ],
  };
}
...
describe("contentDocToFiles", () => {
  ...
  it("carries seo to messages (with ogAlt) and content-spec (title+description only)", () => {
    ...
    expect(messages.seo.ogAlt).toBe("Curso Exemplo");
  ...
  it("carries the brand logo and og image to content-spec/messages when present (ADR 0018)", () => {
    const doc = ccaDoc();
    doc.settings.logo = "https://x.supabase.co/storage/v1/object/public/landing-assets/curso-exemplo/logo.png";
    doc.settings.seo.ogImage = "https://x.supabase.co/storage/v1/object/public/landing-assets/curso-exemplo/og.png";
  ...
  it("carries multi-pixel tracking arrays through to content-spec (SPEC-015)", () => {
    const doc = ccaDoc();
    doc.settings.tracking.meta_pixels = ["123456789012345", "100200300400500"];
    doc.settings.tracking.ga4_ids = ["G-XXXXXXXXXX", "G-ABC1234567"];
    doc.settings.tracking.google_ads_ids = ["AW-123456789"];
    const { contentSpec } = contentDocToFiles(doc);
    expect(contentSpec.tracking.meta_pixels).toEqual(["123456789012345", "100200300400500"]);
    expect(contentSpec.tracking.ga4_ids).toEqual(["G-XXXXXXXXXX", "G-ABC1234567"]);
    expect(contentSpec.tracking.google_ads_ids).toEqual(["AW-123456789"]);
    // legacy single fields remain untouched for back-compat
    expect(contentSpec.tracking.fb_pixel_id).toBe("123456789012345");
    expect(contentSpec.tracking.consent_key).toBe("lp_consent_v1");
  });

  it("carries the public server-tracking endpoint through to content-spec (SPEC-015 §7.1)", () => {
    const doc = ccaDoc();
    doc.settings.tracking.server = { endpoint: "https://track.example.com", lp_id: "11111111-2222-3333-4444-555555555555" };
    const { contentSpec } = contentDocToFiles(doc);
