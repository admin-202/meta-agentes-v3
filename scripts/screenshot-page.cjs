#!/usr/bin/env node
/*
 * screenshot-page.cjs — server-side landing-page review screenshots for Nexus's autonomous
 ...
 * PRIVATE `nexus-review` Storage bucket, and prints a one-line JSON manifest to stdout.
 *
 * Usage:
 ...
 *
 * Security:
 *   - SSRF guard: only https URLs whose host ends with `.example.com` (the Cloudflare Pages domain
...
const ALLOWED_HOST_SUFFIX = ".example.com";
const BUCKET = "nexus-review";
...
function assertAllowedUrl(raw) {
  ...
  // Must be a subdomain of example.com (e.g. promo.example.com) — never the bare apex, never anything
