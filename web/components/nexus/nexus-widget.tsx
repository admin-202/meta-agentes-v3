...
export function NexusWidget() {
  ...
  // state: it appears in fullscreen when Nexus's request_live_review tool fires.
  const liveReview = (
  ...
  return (
    <>
      ...
      <button
        ...
        title="Compartilhe a tela uma vez; depois o Nexus consegue olhar quando você pedir."
      >
        {sharing ? "Nexus está vendo sua tela" : "Nexus pode ver minha tela"}
      </button>
      ...
      <button
        ...
        title="Quando uma landing page terminar de ser criada, o Nexus abre a revisão visual ao vivo automaticamente (compartilhe a tela para rodar sem cliques)."
      >
      ...
      <div className="mt-3 flex items-center justify-between gap-3 border-t border-white/10 pt-2 font-mono text-[10px] uppercase tracking-[0.16em] text-white/35">
        <span>PTT</span>
        <span>{sharing ? "Tela ON" : state.wakeActive ? 'Diga "Nexus"' : state.handsFree ? "Mic ativo" : "Manual"}</span>
      </div>
