...
export type NexusStatus =
  ...
  | "transcribing"
  | "thinking"
  | "capturing" // grabbing a screen frame for Nexus to look at
  | "speaking"
  | "error";
...
const NO_SCREEN_MSG =
  "Não consigo ver sua tela. Ativa o compartilhamento ali no painel do Nexus e me pede de novo.";
...
export function useNexusVoice() {
  ...
  // Nexus edits landing-page drafts straight in Supabase, so the open editor would
  ...
  const pollNarrations = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/nexus/narrations?session=${encodeURIComponent(getSessionId())}`);
      if (!res.ok) return;
    ...
    void fetch(`/api/nexus/narrations/${next.id}`, { method: "PATCH" }).catch(() => {});
  ...
  // Sends the transcript to Nexus and resolves any capture_screen pauses: when the
  ...
  const resolveReply = useCallback(
    async (text: string): Promise<string> => {
      ...
      let data = (await chatRes.json()) as NexusApiResponse;
  ...
  const persistAutoReviewed = useCallback(() => {
    try {
      localStorage.setItem("nexus_autoreviewed_ids", JSON.stringify([...autoReviewedIdsRef.current]));
    } catch {
  ...
  const pollLiveReviewCandidate = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/nexus/live-review/candidate");
      if (!res.ok) return;
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem("nexus_autoreviewed_ids");
      if (raw) {
