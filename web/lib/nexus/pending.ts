...
import type { AgentTrigger, LandingEditSignal, LiveReviewSignal } from "@/lib/nexus/agent-trigger";
import type { ChatTurn } from "@/lib/nexus/memory";
 ...
 * the client, and rebuild it on the follow-up /nexus/capture call. Short-lived:
...
function pendingKey(sessionId: string, id: string): string {
  return `nexus:pending:${sessionId}:${id}`;
}
...
export async function savePending(sessionId: string, id: string, state: PendingTurn): Promise<void> {
  try {
    ...
    console.warn(
      JSON.stringify({ level: "warn", event: "nexus_pending_unavailable", op: "save", message: errMsg(err) }),
    );
...
export async function loadPending(sessionId: string, id: string): Promise<PendingTurn | null> {
  try {
    ...
    console.warn(
      JSON.stringify({ level: "warn", event: "nexus_pending_unavailable", op: "load", message: errMsg(err) }),
    );
    return null;
  }
}
