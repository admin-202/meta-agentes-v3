...
const CREATE_SKILL_BY_SLUG: Record<string, string> = {
  "cliente-exemplo": "create-traffic-cliente-exemplo-campaign",
};
...
const CREATE_SALES_SKILL_BY_SLUG: Record<string, string> = {
  "cliente-exemplo": "create-sales-cliente-exemplo-campaign",
};
const ACTIVATE_SKILL_BY_SLUG: Record<string, string> = {
  "cliente-exemplo": "activate-campaign-cliente-exemplo",
};
const ANALYZE_SKILL_BY_SLUG: Record<string, string> = {
  "cliente-exemplo": "funnel-analytics-cliente-exemplo-campaign",
};
const LANDING_SKILL_BY_SLUG: Record<string, string> = {
  "cliente-exemplo": "create-landing-page-cliente-exemplo",
};
const PUBLISH_SKILL_BY_SLUG: Record<string, string> = {
  "cliente-exemplo": "publish-landing-page-cliente-exemplo",
};

// `nome` becomes the subdomain (<nome>.example.com) AND the Cloudflare project suffix
// (lp-<nome>) AND a runner arg — so it must satisfy the poller's args charset and
...
/** Append-only audit trail for Nexus's autonomous landing-page actions (Repudiation —
...
async function logLandingOp(clientId: string, lpId: string, summary: string): Promise<void> {
  const res = await db()
    .from("operation_logs")
    .insert({ client_id: clientId, entity_type: "landing_page", entity_id: lpId, action: "update", actor: "nexus", summary });
  if (res.error) {
...
const tools: Record<string, ToolDef> = {
  ...
  request_landing_page_creation: {
    spec: {
      ...
      description:
        "Enfileira a CRIAÇÃO de uma landing page profissional para um cliente (os agents rodam na VM, fazem deploy no Cloudflare Pages sob <nome>.example.com). A página nasce em PREVIEW (noindex, não indexável) — NÃO gasta verba de anúncio. FLUXO OBRIGATÓRIO: chame primeiro com confirm=false para obter os detalhes, leia-os ao operador e peça confirmação; só chame com confirm=true após um 'sim' explícito.",
      input_schema: {
        type: "object",
        properties: {
          client_slug: { type: "string", description: "slug do cliente, ex.: cliente-exemplo" },
          nome: {
            type: "string",
            description:
              "rótulo do subdomínio (vira <nome>.example.com), só minúsculas, números e hífen, 2-40 chars, ex.: 'promo'. OBRIGATÓRIO: se o operador não informou, PERGUNTE qual subdomínio usar antes de chamar — nunca invente nem reutilize o de uma página existente.",
          },
          confirm: {
            type: "boolean",
            description: "false = apenas devolve os detalhes para confirmar; true = enfileira de fato (use só após o operador confirmar)",
    ...
    handler: async (input) => {
      ...
      // guessed — guessing could target a live page (e.g. curso-exemplo.example.com). See ADR 0012.
      if (!nomeRaw) {
        return {
          needs_input: true,
          field: "nome",
          message:
            "Preciso do nome do subdomínio para a landing page (ex.: 'promo' vira promo.example.com). Pergunte ao operador qual usar — não invente um nome nem reutilize o de uma página existente.",
        };
      }
  ...
  list_landing_pages: {
    spec: {
      ...
      input_schema: {
        ...
        properties: {
          client_slug: { type: "string", description: "slug do cliente, ex.: cliente-exemplo" },
          product_slug: { type: "string", description: "opcional: filtra por produto, ex.: curso-exemplo" },
        },
        required: ["client_slug"],
      },
    },
    ...
    handler: async (input) => {
      ...
      if (!confirm) {
        return {
          ...
          subdomain: `${lp.subdomain}.example.com`,
          section: type,
      ...
      for (let attempt = 0; attempt < 2; attempt++) {
        const upd = await db()
          .from("landing_page_sections")
          .update({ fields: toFields as Json, version: version + 1, updated_by: "nexus" })
          .eq("landing_page_id", id)
          .eq("type", type)
          .eq("version", version)
          .select("version")
  ...
  request_landing_page_publish: {
    spec: {
      ...
      description:
        "Enfileira a PUBLICAÇÃO de uma landing page no Cloudflare (os agents serializam o rascunho atual, buildam e fazem deploy sob <subdomínio>.example.com). Por padrão republica mantendo o noindex atual; passe noindex=false para go-live (indexável). FLUXO em dois passos: confirm=false devolve os detalhes; confirm=true enfileira após o 'sim'.",
      input_schema: {
        type: "object",
        properties: {
          landing_page_id: { type: "string", description: "uuid da LP (use list_landing_pages)" },
  ...
  start_autonomous_mode: {
    spec: {
      ...
      description:
        "Liga o MODO AUTÔNOMO: o Nexus passa a monitorar sozinho uma tarefa longa que JÁ FOI enfileirada (hoje: a CRIAÇÃO de uma landing page) e vai te narrando o progresso por voz, periodicamente, até concluir. Use quando o operador disser algo como 'vou ter que sair, inicia o modo autônomo e monitora a execução'. PRÉ-REQUISITO: já deve existir um pedido de criação de landing page recente para o cliente (via request_landing_page_creation) — este tool NÃO cria nada, só passa a observar. Risco baixo, sem gasto: NÃO precisa de confirmação em dois passos.",
      input_schema: {
        type: "object",
        properties: {
          client_slug: { type: "string", description: "slug do cliente, ex.: cliente-exemplo" },
