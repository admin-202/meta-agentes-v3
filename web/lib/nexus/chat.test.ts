...
vi.mock("@/lib/nexus/pending", () => ({ savePending, loadPending, deletePending }));
...
describe("runChat — agent trigger metadata", () => {
  it("exposes a trigger when an Nexus write tool enqueues a job", async () => {
...
describe("resumeChat — chain capture into a data tool", () => {
  it("after seeing the screen, calls a server-side data tool and answers", async () => {
    ...
    createMock
      .mockResolvedValueOnce(toolTurn("tu_metrics", "get_campaign_metrics", { client_slug: "cliente-exemplo" }))
...
describe("runChat — mixed turn (capture_screen + data tool together)", () => {
  it("runs the data tool now, defers capture, recombines both results on resume", async () => {
    createMock.mockResolvedValueOnce({
      stop_reason: "tool_use",
      content: [
        { type: "tool_use", id: "tu_ov", name: "get_client_overview", input: { client_slug: "cliente-exemplo" } },
        { type: "tool_use", id: "tu_cap", name: "capture_screen", input: {} },
      ],
    });

    const paused = await runChat(SESSION, "o que tem nessa tela do cliente?");
    if (paused.kind !== "need_capture") throw new Error("expected need_capture");
    expect(runTool).toHaveBeenCalledWith(
      "get_client_overview",
      { client_slug: "cliente-exemplo" },
