...
const CLIENT = { id: "client-uuid", name: "Cliente", currency: "BRL", daily_budget_cap_cents: 5000 };
...
describe("start_autonomous_mode", () => {
  ...
  it("returns started=false when there is no recent landing job to watch", async () => {
    S.client = CLIENT;
    S.job = null;
    const out = (await runTool("start_autonomous_mode", { client_slug: "cliente-exemplo" }, CTX)) as Record<string, unknown>;
