...
describe("validateSectionFields", () => {
  ...
  it("rejects a javascript: href but accepts http(s)/relative/mailto", () => {
    ...
    expect(validateSectionFields({ footer: { href: "https://example.com" } }).ok).toBe(true);
...
describe("settingsPatchSchema", () => {
  ...
  it("accepts well-formed tracking ID arrays (SPEC-015)", () => {
    const r = settingsPatchSchema.safeParse({
      tracking: {
        meta_pixels: ["123456789012345", "100200300400500"],
        ga4_ids: ["G-XXXXXXXXXX"],
  ...
  it("rejects malformed tracking IDs, oversized arrays and consent_key injection", () => {
    ...
    expect(
      settingsPatchSchema.safeParse({ tracking: { meta_pixels: new Array(11).fill("123456789012345") } }).success,
    ).toBe(false);
...
describe("trackingSecretsSchema (Phase 2 — write-only secrets)", () => {
  it("accepts well-formed meta/ga4/google_ads secret entries", () => {
    const r = trackingSecretsSchema.safeParse({
      entries: [
        { provider: "meta", public_id: "123456789012345", secret: { capi_token: "EAABsbCS1iHgBA_long_token_value" }, test_event_code: "TEST12345" },
        { provider: "ga4", public_id: "G-XXXXXXXXXX", secret: { api_secret: "abc123_apisecret_value" } },
        {
          provider: "google_ads",
  ...
  it("rejects unknown keys, malformed ids, and empty/oversized payloads", () => {
    ...
    expect(
      trackingSecretsSchema.safeParse({ entries: [{ provider: "meta", public_id: "123456789012345", secret: { capi_token: "tokenvalue1" }, evil: 1 }] }).success,
    ).toBe(false);
    // unknown key inside secret (.strict) — can't smuggle extra fields
    expect(
      trackingSecretsSchema.safeParse({ entries: [{ provider: "meta", public_id: "123456789012345", secret: { capi_token: "tokenvalue1", extra: "x" } }] }).success,
    ).toBe(false);
    // malformed pixel id
    expect(
    ...
    expect(
      trackingSecretsSchema.safeParse({ entries: [{ provider: "meta", public_id: "123456789012345", secret: { capi_token: "has space" } }] }).success,
    ).toBe(false);
    // empty entries
    ...
    expect(
      trackingSecretsSchema.safeParse({ entries: [{ provider: "ga4", public_id: "G-XXXXXXXXXX", secret: { capi_token: "tokenvalue1" } }] }).success,
    ).toBe(false);
  });

  it("validates the delete payload (provider + public_id only)", () => {
    expect(trackingSecretDeleteSchema.safeParse({ provider: "meta", public_id: "123456789012345" }).success).toBe(true);
    expect(trackingSecretDeleteSchema.safeParse({ provider: "ga4", public_id: "G-XXXXXXXXXX" }).success).toBe(true);
