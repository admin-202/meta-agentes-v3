...
const nextConfig: NextConfig = {
  ...
  // components the static template uses, imported from the shared @template/lp-render
  // package (a file: dependency, not pre-compiled). Next must transpile it. See ADR 0017.
  transpilePackages: ["@template/lp-render"],
  ...
  // @template/lp-render is a symlinked file: dep. By default webpack resolves it to its realpath
  ...
  webpack: (config) => {
    ...
    // With symlinks=false, webpack sees @template/lp-render at its node_modules symlink path,
