...
Server-side tracking para TODAS as landing pages `*.example.com`, sem Stape e sem um Worker por
LP. Um único Worker em **`track.example.com`** (same-site) resolve os segredos de cada página por
...
```
LP (*.example.com)  --POST /e {lp_id, event_id, ...}-->  track.example.com (este Worker)
   ...
   • seta _fbp/_fbc first-party (Domain=.example.com) — sobrevive ao ITP
...
# 3. Ajuste a rota (track.example.com) no wrangler.toml e:
wrangler deploy
```

No painel Cloudflare: `track.example.com` **proxied** (nuvem laranja) e **SSL/TLS = Full**.
...
- `GET https://track.example.com/healthy` → `ok`
...
- CORS restrito por **sufixo** de origin (`.example.com`); rate-limit por IP.
