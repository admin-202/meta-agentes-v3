...
// O ganho é que o Set-Cookie vem do SERVIDOR first-party. Domain=.example.com porque o Worker
// é same-site de todas as LPs (*.example.com) → cookie first-party que sobrevive ao ITP.
export function fpCookie(env: Env, name: string, value: string, days = 90): string {
  const domain = env.ALLOWED_ORIGIN_SUFFIX.replace(/^\.?/, "."); // ".example.com"
