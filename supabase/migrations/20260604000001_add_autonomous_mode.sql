...
-- ADR: docs/adr/0019-nexus-autonomous-mode.md
-- Spec: docs/specs/SPEC-013-nexus-autonomous-mode.md
--
-- "Modo autônomo" do Nexus: depois que o operador enfileira uma tarefa longa (ex.: gerar
-- uma landing page) e sai, o Nexus monitora a execução sozinho, narra o progresso por voz,
...
--   * nexus_narrations — o CANAL servidor→browser. A skill headless insere a fala; a aba do
--     Nexus faz polling (GET /api/nexus/narrations) e fala via TTS. Reusa o padrão da ADR
...
create table public.autonomous_watches (
  ...
  -- A aba do navegador (sessionStorage do Nexus) que deve FALAR as narrações deste watch.
  ...
  started_by    text not null default 'nexus',
...
-- ---------- nexus_narrations ----------
-- Append-only. Uma linha = uma fala que a aba do Nexus deve dizer (TTS). spoken_at é marcado
...
create index nexus_narrations_session_ts_idx on public.nexus_narrations (session_id, ts);
create index nexus_narrations_watch_idx on public.nexus_narrations (watch_id);
