...
description: Gera de forma 100% autônoma e headless um resumo diário (por IA) do que os agents fizeram para o cliente cliente-exemplo — lê operation_logs + analyses + estado das campanhas do dia no Supabase e faz upsert em daily_summaries. Usado pelo Nexus (dashboard) para responder "o que foi feito hoje?" em uma query. Use quando pedirem "gerar resumo diário do cliente-exemplo", ou quando disparada via cron à noite (`claude -p --dangerously-skip-permissions ".claude/skills/daily-summary-cliente-exemplo"`).
---

# Skill: /daily-summary-cliente-exemplo

Gera o resumo diário do dia corrente (timezone America/Sao_Paulo) para o cliente
`cliente-exemplo` e persiste em `public.daily_summaries`. É **read-only sobre a conta
Meta** — não lê a Meta; apenas consolida o que já está no Supabase. Alimenta o
assistente Nexus do dashboard (tool `get_daily_summary`).

> Spec: docs/specs/web-dashboard-nexus.md · ADR: docs/adr/0007-daily-summaries-and-agent-events.md
...
```sql
select id from public.clients where slug = 'cliente-exemplo';
```
...
`{ "skill":"daily-summary-cliente-exemplo","client":"cliente-exemplo","date":"<summary_date>","verified":true,"actions_count":N,"latest_verdict":"...","persisted":true }`.
...
- 1 linha em `public.daily_summaries` para `(cliente-exemplo, hoje)`, com `summary`
...
## 6. Pré-requisitos
- Schema base + seed do cliente `cliente-exemplo` aplicados (migrations versionadas).
