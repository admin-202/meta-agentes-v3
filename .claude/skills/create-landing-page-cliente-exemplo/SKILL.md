...
description: Gera de forma 100% autônoma e headless o RASCUNHO de uma landing page profissional de alta conversão para um PRODUTO do cliente cliente-exemplo (catálogo em lista-de-produtos) e o escreve AO VIVO no Supabase como blocos editáveis (landing_pages.settings + .theme + landing_page_sections), depois ENFILEIRA a publicação (job landing_publish) que faz build + deploy no Cloudflare Pages. Fluxo: brief do produto (catálogo) → arquitetura de conversão → copy long-form pt-BR → hero/OG → escrita ao vivo no Supabase → enfileira publish. Use quando pedirem "criar landing page para cliente-exemplo" (ex.: produto curso-exemplo ou workshop-exemplo), ou quando disparada via Nexus/headless (`claude -p --dangerously-skip-permissions ".claude/skills/create-landing-page-cliente-exemplo product=curso-exemplo nome=curso-exemplo"`). NÃO faz build nem deploy aqui (isso é do job landing_publish / skill publish-landing-page-*). NÃO cria campanha Meta.
...
# Skill: /create-landing-page-cliente-exemplo

Gera, **de ponta a ponta e sem intervenção humana**, o **rascunho editável** de uma landing
page profissional de alta conversão para o cliente **cliente-exemplo** e **enfileira a
publicação** no Cloudflare Pages sob `<nome>.example.com`:
...
> linhas `landing_page_sections` (uma por bloco). O operador (UI) e o Nexus (voz) editam
...
3. **Cliente é fixo: `cliente-exemplo`.** Não generalize.
...
lookup de `clients WHERE slug='cliente-exemplo'` no Supabase (REST) para o `client_id` (uuid) —
**não hardcode**.

| Campo | Valor |
|---|---|
| slug | `cliente-exemplo` |
| Domínio | `<nome>.example.com` (zona `example.com` na conta CF) |
| Materiais | `.claude/materiais-das-empresas/cliente-exemplo/` (logo, mascote, exemplo-de-ads, **produtos/**) |
...
o `.claude/` é COPY-ado para a imagem Fly). O arg `product=<slug>` seleciona qual (default `curso-exemplo`).
...
invente** dados de produto — use o brief. Produtos atuais: `curso-exemplo` e `workshop-exemplo`.
...
|---|---|---|
| `product` (slug do catálogo) | `curso-exemplo` (default) | Seleciona o brief `${MAT}/produtos/${product}.json`. Se o arquivo não existir → aborta (`verified:false`). |
| `nome` (subdomínio) | **obrigatório (sem default)** | Vira `<nome>.example.com` + projeto CF `lp-<nome>` + `landing_pages.subdomain`. Sem `nome` → aborta. **Nunca** assuma `curso-exemplo` (é uma página de produção). O brief tem `defaultSubdomain`, mas `nome` ainda precisa ser passado explicitamente. |
...
**Args** via `$ARGUMENTS` (`key=value`): `nome` (**obrigatório**), `product` (default `curso-exemplo`),
`ref-url` (opcional), `cart-state`, `noindex`. Sem `nome` → aborta (manifest `verified:false`).
Nunca use `curso-exemplo` como fallback de `nome`. `checkout-url`/`cart-state`/`deadline` vêm do brief do
...
- Parse dos args; aplicar defaults da §3 (`product=curso-exemplo`). **`nome` é obrigatório**: se ausente
  → manifest `verified:false` (`errors:["nome obrigatório"]`) e sair. Validar
  `nome =~ ^[a-z0-9-]{2,40}$` e `product =~ ^[a-z0-9-]{2,40}$`. **Nunca** assumir `curso-exemplo` como `nome`.
- Paths: `LP_DIR="${REPO}/landing-pages/${nome}"`, `TRY_DIR=tentativas-geracao-de-campanhas`,
  `MAT=.claude/materiais-das-empresas/cliente-exemplo`, `BRIEF_FILE="${MAT}/produtos/${product}.json"`.
...
- **Resolver os assets do brief (ADR 0014/0018) — fonte de verdade é `assets.*`, com fallback
  ...
  INSTRUCTOR_SRC=$(resolve_asset '.assets.instructorPhoto' "${MAT}/logo/foto-do-infoprodutor/instrutor.png")
...
- **Constantes derivadas:**
  ...
  META_PIXELS='["<META_PIXEL_ID>"]'
  GA4_IDS='["<GA4_MEASUREMENT_ID>"]'
  GOOGLE_ADS_IDS='[]'
  TRACKING=$(jq -cn \
    --argjson mp "${META_PIXELS}" --argjson g4 "${GA4_IDS}" --argjson ga "${GOOGLE_ADS_IDS}" \
    '{fb_pixel_id: ($mp[0] // ""), ga4_id: ($g4[0] // ""), consent_key: "lp_consent_v1",
...
1. **Client lookup** (REST):
   ```bash
   CLIENT=$(curl -fsS "${REST}/clients?slug=eq.cliente-exemplo&select=id,materials_path" \
     -H "apikey: ${SUPABASE_KEY}" -H "Authorization: Bearer ${SUPABASE_KEY}" --max-time 15)
   CLIENT_ID=$(echo "${CLIENT}" | jq -r '.[0].id // empty')
   ```
   Vazio → manifest `verified:false` (`errors:["cliente cliente-exemplo não encontrado"]`), sair.
...
4. **`settings` parcial** (o resto — seo/cartClosed — entra no Passo 4, quando a copy existe):
   ...
   SETTINGS=$(jq -nc --arg sub "${nome}" --arg name "${SHORT}" --arg product "${PROD_NAME}" \
     --arg site "https://${nome}.example.com" --argjson price "${PRICE_CENTS:-null}" \
...
5. **Upsert `landing_pages`** `ON CONFLICT (subdomain)` (colunas NOT NULL: client_id, name,
   ...
   LBODY=$(jq -nc --arg cid "${CLIENT_ID}" \
     ...
     --arg name "${SHORT}" --arg sub "${nome}" --arg fqdn "${nome}.example.com" \
     --arg url "https://${nome}.example.com" --arg repo "landing-pages/${nome}" \
     --arg cfp "lp-${nome}" --argjson theme "${THEME}" --argjson settings "${SETTINGS}" \
...
- **No runner Fly** o `_template` já traz `node_modules` pré-bakeado (inclui `tsx` + o symlink
  `@template/lp-render`); o `cp` os leva junto → o job `landing_publish` (mesma máquina) acha
...
2c. **Retrato do hero (lado direito, layout split):** se `HERO_IMG_SRC` (Passo 0) existe, copie-o
   ...
   inalterado para produtos sem `assets.heroImage` (ex.: `curso-exemplo`).
2d. **Modelo 3D do painel (acima do hero):** se `STAGE_MODEL_SRC` (Passo 0) existe, ele será
   **subido direto pro Storage** no Passo 4 (NÃO copie pro `public/` — o painel carrega da URL do
   ...
   fica ausente e a página não tem painel (inalterado p/ `curso-exemplo`).
...
2. **Enfileirar `landing_publish`** (INSERT em `agent_jobs`; o poller do Fly dispara a skill
   `publish-landing-page-cliente-exemplo`, que serializa→build→deploy). O dedup per-LP
   ...
   JOB=$(jq -nc --arg cid "${CLIENT_ID}" --arg lp "${LP_ID}" --arg ni "${noindex:-1}" \
     '{client_id:$cid, skill:"publish-landing-page-cliente-exemplo", kind:"landing_publish",
...
3. **`operation_logs`** — uma linha (sem segredos):
   ...
   curl -fsS -X POST "${REST}/operation_logs" \
     ...
     -d "$(jq -nc --arg c "${CLIENT_ID}" --arg e "${LP_ID}" \
         --arg s "LP ${nome}.example.com: rascunho gerado (${N_SECTIONS} seções) e publish enfileirado (noindex=${noindex:-1})" \
...
LP id, subdomínio (`https://${nome}.example.com`), nº de seções gravadas, `draft_status='ready'`,
...
- ❌ Assumir `nome=curso-exemplo` (ou qualquer default) — `nome` é obrigatório; sem ele, aborte.
...
O publish (skill `publish-landing-page-cliente-exemplo`) faz scaffold-se-preciso, serializa,
...
pré-bakeado (com `tsx` + symlink `@template/lp-render`); o job publish (mesma máquina) reusa e
...
- **Brief do produto no catálogo**: `${MAT}/produtos/${product}.json` (skill `lista-de-produtos`,
  ADR 0014). Sem ele, a skill aborta. Produtos atuais: `curso-exemplo`, `workshop-exemplo`.
- Skill `publish-landing-page-cliente-exemplo` no disco (o poller a dispara pelo job).
