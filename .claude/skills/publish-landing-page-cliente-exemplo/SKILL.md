...
description: Publica (deploy no Cloudflare Pages) o RASCUNHO atual de uma landing page do cliente cliente-exemplo que vive no Supabase (landing_pages.settings + landing_pages.theme + landing_page_sections). Serializa o ContentDoc → messages/pt.json + content-spec.json + theme.css (serializer puro compartilhado) → next build (static export) → wrangler deploy → persiste o snapshot publicado. Use quando pedirem "publicar a landing page X" ou quando disparada via Nexus/headless pela fila agent_jobs (kind=landing_publish, ex.: `claude -p --dangerously-skip-permissions ".claude/skills/publish-landing-page-cliente-exemplo landing_page_id=<uuid>"`). NÃO gera copy nem cria a LP — só publica o rascunho existente. NÃO cria campanha Meta.
...
gerando um **snapshot estático** no Cloudflare Pages sob `<subdomain>.example.com`.
...
> 0017 (pacote `@template/lp-render`). O bloco de deploy reusa o **Passo 9** de
> `create-landing-page-cliente-exemplo` (mesma mecânica de Cloudflare/DNS/SSL).
...
1. **Linha da LP** — por id (preferido) ou por subdomínio:
   ...
   - Extraia: `LP_ID=.[0].id`, `CLIENT_ID=.[0].client_id`, `SUB=.[0].subdomain`,
     `CF_PROJECT=.[0].cloudflare_project_id // "lp-"+SUB`, `SETTINGS=.[0].settings`,
...
- **Scaffold** se ausente (cópia do template, que já traz `node_modules` pré-bakeado no
  runner — inclui `tsx` e o symlink `@template/lp-render`):
  ```bash
...
- Leia `${LP_DIR}/app/layout.tsx`. Se **não** contém `import "./theme.css";`, insira essa
  linha **imediatamente após** `import "@template/lp-render/globals.css";` (use `Edit`).
...
`<SUB>.example.com` — republicar é a função desta skill. **Não** aborte por "já existe deploy".
...
Mesma mecânica do **Passo 9** de `create-landing-page-cliente-exemplo` (ver lá os detalhes/
gotchas de DNS/SSL). Use `CF_TOKEN` + `CLOUDFLARE_ACCOUNT_ID`. Em `${LP_DIR}`:
```bash
PROJ="${CF_PROJECT}"   # geralmente lp-${SUB}
...
ZID=$(curl -sS "https://api.cloudflare.com/client/v4/zones?name=example.com" \
...
for i in $(seq 1 8); do
  CODE=$(curl -sS -o /dev/null -w "%{http_code}" "https://${SUB}.example.com/" || echo 000)
...
- **`operation_logs`** — uma linha (sem segredos):
  ...
  curl -fsS -X POST "${REST}/operation_logs" \
    -H "apikey: ${SUPABASE_KEY}" -H "Authorization: Bearer ${SUPABASE_KEY}" \
    -H "Content-Type: application/json" -H "Prefer: return=minimal" --max-time 15 \
    -d "$(jq -nc --arg c "${CLIENT_ID}" --arg e "${LP_ID}" --arg s "LP ${SUB}.example.com publicada (noindex=${NOINDEX_FLAG}, ssl=${SSL})" \
...
- `https://<SUB>.example.com/` → `200` (ou `ssl_pending` documentado).
...
- **Scaffold preserva `node_modules` do `_template`** (com `tsx` + symlink `@template/lp-render`)
