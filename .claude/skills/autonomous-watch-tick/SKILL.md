...
curl -fsS "${H[@]}" "${BASE}/storage/v1/object/nexus-review/${SHOT_PATH}" -o /tmp/rev.jpg --max-time 20
```
...
1. **Compor o email** (sem PII além da URL pública + resumo). Escreva o corpo num arquivo temporário
   ...
   printf '%s\n' "A landing page do produto <produto/subdomínio> está no ar:" "" "${URL}" "" \
     "<uma a duas linhas de impressão geral da revisão>" "" "— Nexus" > /tmp/mail-body.txt
...
curl -fsS -X POST "${H[@]}" -H "Content-Type: application/json" -H "Prefer: return=minimal" \
  "${REST}/nexus_narrations" -d "$BODY" --max-time 10
```
...
- No máximo **uma** linha nova em `nexus_narrations` para a `session_id` do watch.
...
- ❌ Capturar print de qualquer URL fora de `*.example.com` — o screenshotter já recusa (SSRF guard).
...
## 6. Pré-requisitos
- Tabelas `autonomous_watches` + `nexus_narrations` e RPC `claim_autonomous_watch`
  (migration `20260604000001_add_autonomous_mode`).
- Bucket privado de Storage `nexus-review` (migration `20260604000002_add_nexus_review_bucket`).
...
- Enviador de email `scripts/send-email.cjs` (Resend) + secret `RESEND_API_KEY` no Fly. Opcional:
  `AUTONOMOUS_NOTIFY_EMAIL` (default `<OPERATOR_EMAIL>`) e `AUTONOMOUS_FROM_EMAIL`
  (default `Nexus <<FROM_EMAIL>>`). Sem a key, o Passo N degrada (narra e encerra, sem email).
