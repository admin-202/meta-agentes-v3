...
description: Cria de forma 100% autônoma e headless uma campanha de VENDAS (OUTCOME_SALES, CBO, otimizada por PURCHASE no pixel) para o cliente cliente-exemplo REUSANDO os criativos "top vendas" já vencedores da conta — descobre os ads que mais venderam (get_insights ad-level + list_ads → creative_id), cria campanha + ad set (OFFSITE_CONVERSIONS) + N ads PAUSED via o MCP "META ADS MCP", persiste no Supabase e escreve manifest. NÃO gera arte nem copy — reaproveita creative_id existentes. Use quando pedirem "criar campanha de vendas com os top criativos do cliente-exemplo", ou via headless (`claude -p --dangerously-skip-permissions ".claude/skills/create-sales-cliente-exemplo-campaign"`).
...
# Skill: /create-sales-cliente-exemplo-campaign

Cria, **de ponta a ponta e sem intervenção humana**, uma campanha de **vendas
(conversões/compra)** no Meta Ads para o cliente **cliente-exemplo**, **reaproveitando os
...
É a **irmã de vendas** da `/create-traffic-cliente-exemplo-campaign`. Diferenças-chave:
...
**omite `destination_type`**, e usa o **novo MCP "META ADS MCP"**.

> Base humana testada: `how-to/criar-campanha-vendas-top-creativos.md` (run real
> 15/jun/2026, 3 gates da Meta). Toda a inteligência mora aqui; um runner/Nexus só dispara
> `claude -p --dangerously-skip-permissions ".claude/skills/create-sales-cliente-exemplo-campaign"`.
...
4. **Meta só via o MCP "META ADS MCP".** **Persista tudo no Supabase via MCP.**
...
da linha `clients WHERE slug='cliente-exemplo'` no Supabase para obter o `client_id` (uuid) —
**não hardcode o uuid**.
...
| Facebook Page | `<FACEBOOK_PAGE_ID>` ("Cliente Exemplo") |
| **Pixel (conversão)** | `<META_PIXEL_ID>` |
| Budget cap | `5000` cents (R$50/dia) · moeda `BRL` |
| Razão social (DSA) | `Acme` *(placeholder — confirmar; só usado numa futura variante BR)* |
| Ads Manager URL | `https://business.facebook.com/adsmanager/manage/campaigns?act=<AD_ACCOUNT_ID>` |

> ⚠️ **Discrepância de `business_id` (não-bloqueante):** o operador informou
> `201551218796069` para este MCP; a `lista-de-clientes` registra a BM `<BUSINESS_MANAGER_ID>`.
...
| promoted_object | `{pixel_id:<META_PIXEL_ID>, custom_event_type:"PURCHASE"}` | Otimiza por compra |
...
> (`B2_Tech_Meta_Ads` → `MCP_META_PRO` → `META_ADS_MCP_B2_TECH`, how-to §6). As tools
...
- `list_ad_accounts` (ou `list_campaigns(account_id="act_<AD_ACCOUNT_ID>")`) → confirmar que
  a conta `<AD_ACCOUNT_ID>` responde. Se não → manifest `verified:false`
  (reason `account_unreachable`) e sair (nada criado, custo zero).

### Passo 2 — Descobrir os criativos vencedores
- `get_insights(object_id="act_<AD_ACCOUNT_ID>", level="ad", date_preset="<date-preset>",
...
- Para cada `ad_id` vencedor, `list_ads(account_id="act_<AD_ACCOUNT_ID>", ...)` (ou filtrado)
...
`create_campaign` (conta `act_<AD_ACCOUNT_ID>`):
...
| **1885499** (`No Advertiser Permission On Page`) | token sem task *Advertise* na Página | **abortar limpo** + registrar passo manual (conceder permissão de Anunciante na Página `<FACEBOOK_PAGE_ID>` no Business Manager) e refazer depois |
...
- `creatives` (conflict `meta_creative_id`) — **uma linha por creative_id reusado** para que
  `ads.creative_id` (FK) resolva: `client_id, meta_creative_id, page_id='<FACEBOOK_PAGE_ID>',
...
```json
{
  ...
  "ad_set": {"meta_ad_set_id":"...","optimization_goal":"OFFSITE_CONVERSIONS","pixel_id":"<META_PIXEL_ID>","custom_event_type":"PURCHASE","geo":["US"],"destination_type":null},
...
- 1 campanha SALES + 1 ad set (`OFFSITE_CONVERSIONS` + pixel PURCHASE) + N ads **PAUSED** na
  conta `<AD_ACCOUNT_ID>`, nomes `[SALES][TopCreatives]...`, geo `US`, **sem `destination_type`**.
...
não tem a task *Advertise* na Página `<FACEBOOK_PAGE_ID>`, o `create_ad` falha. Não dá pra
...
- MCP "META ADS MCP" conectado, token válido com `ads_management`
  (`meta_token_status` → `is_valid:true`; senão `meta_login`).
- Pixel `<META_PIXEL_ID>` ativo na conta.
- Permissão de Anunciante na Página `<FACEBOOK_PAGE_ID>` ("Cliente Exemplo").
