...
> ADR: [0010-nexus-screen-vision](../adr/0010-nexus-screen-vision.md) ·
> Threat model: [nexus-screen-vision](../security/threats/nexus-screen-vision.md)

## Objetivo

Dar ao Nexus (assistente de voz do dashboard) a capacidade de **ver a tela do
...
- **Captura:** Screen Capture API (`getDisplayMedia`) com **stream persistente**.
  ...
  na web). O operador compartilha **uma vez por sessão**; a partir daí o Nexus
...
`key = nexus:pending:{sessionId}:{id}`. Campos: `messages` (histórico até o turno
...
| Caso | Comportamento |
|------|---------------|
| Operador não compartilhou a tela | `captureFrame()` retorna null → Nexus pede para ativar o compartilhamento; não inventa |
...
1. Pedir "que erro é esse na tela?" com a tela compartilhada → Nexus descreve o que
   está na tela (via `capture_screen` → resume).
2. "Analisa a campanha que estou vendo" → Nexus vê, identifica e chama uma tool de
   dados no mesmo resume (`usedTools` contém `capture_screen` + tool de dados).
3. Sem compartilhamento ativo → Nexus pede para ativar, sem alucinar conteúdo.
