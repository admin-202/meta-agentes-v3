# SPEC-014 — Nexus Live Review (revisão visual ao vivo, operador presente)
...
| ADR | [0020](../adr/0020-nexus-live-review-client-side.md) |
| Relacionado | [SPEC-013](SPEC-013-nexus-autonomous-mode.md) (revisão headless, operador AUSENTE), [ADR 0010](../adr/0010-nexus-screen-vision.md) (Nexus vê a tela), [ADR 0017](../adr/0017-shared-lp-render-package.md) (lp-render) |
| Afeta | `packages/lp-render/` (ReviewBridge), `web/` (orquestrador + tool + endpoint de visão), template (Surface B opcional) |

## 1. Objetivo

Quando uma landing page fica pronta **com o operador presente** (ex.: gravação), o Nexus faz uma
...
- **Surface B — nova aba da página publicada (URL autêntica `example.com`).** `window.open(url?review=1)`;
...
(origens do dashboard: prod `meta-ads-agents.vercel.app` + previews + localhost dev — via const
...
- Entrada: `startLiveReview({ landingPageId, url?, surface })`, disparada por (a) resultado de uma
  tool do Nexus, ou (b) um botão "revisar ao vivo" no dashboard.
...
- **Loop** (cancelável; cap duro de passos):
  ...
  4. POST `/api/nexus/review-frame` (visão: "descreva e opine sobre esta seção em 1–2 frases,
...
- Tool `request_live_review(landing_page_id)` → retorna `LiveReviewSignal { landingPageId, url }`,
  ...
  `web/lib/nexus/agent-trigger.ts`.)
- Gatilho natural: o operador diz *"Nexus, revisa a página comigo"* logo após a criação, OU o fim
...
- **Info disclosure**: o frame é a tela do próprio operador (já compartilhada ao Nexus por ADR 0010);
...
- Captura de frame: `getDisplayMedia` persistente + `capture_screen` (ADR 0010 / memória
  `nexus-screen-vision`).
- Visão + voz: pipeline de chat/TTS do Nexus (`web/lib/nexus/*`, `use-nexus-voice`).
- Preview real: `web/app/(preview)/lp-preview/[id]` (renderiza lp-render, inclui Stage3D).
- Transporte de sinal: `web/lib/nexus/agent-trigger.ts` (CustomEvent + BroadcastChannel) — espelhar.
...
2. ✅ **Endpoint** `POST /api/nexus/review-frame` (`web/lib/nexus/review-frame.ts` + handler em
   `route.ts`): visão one-shot sonnet, 1–2 frases pt-BR, rate-limit `nexusReview`, validação base64.
3. ✅ **Orquestrador** client: `web/lib/nexus/live-review.ts` (`runLiveReview`, loop cancelável +
   cap 18 passos + timeout 6 min) e `web/components/nexus/live-review-stage.tsx` (overlay
...
4. ✅ **Tool** `request_live_review` (`tools.ts`) + `LiveReviewSignal` (`agent-trigger.ts`) +
   ...
   (`use-nexus-voice.ts`) + montagem do overlay no `nexus-widget.tsx`.
...
- **Reuso de stream/voz**: o overlay reusa `captureFrame`/`speak`/`startShare` expostos pelo
  `use-nexus-voice` — sem segundo prompt de captura nem caminho de TTS duplicado.
