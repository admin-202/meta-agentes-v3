...
| ADR | [docs/adr/0019-nexus-autonomous-mode.md](../adr/0019-nexus-autonomous-mode.md) |
| Escopo v1 | `target_kind = landing_page` (genérico no schema) |
| Handoff | `NODES.md` (raiz) — estado vivo, paths, como testar, próximos passos |

## 1. Objetivo

Permitir que o operador, após enfileirar uma tarefa longa pelo Nexus (ex.: gerar uma landing
page), peça **"inicia o modo autônomo e monitora a execução"** e saia. O Nexus então, sem
...
1. Operador: *"Nexus, cria a landing page do produto `workshop-exemplo`."* → Nexus enfileira
   (`agent_jobs`, ADR 0009).
2. Operador: *"Vou ter que sair. Inicia o modo autônomo e monitora toda a execução."*
3. Nexus chama `start_autonomous_mode(target_kind='landing_page', target_id=<lp>)` →
...
6. Nexus abre a URL (print server-side), fala opinião do topo, rola ~25%, novo print, nova
...
`autonomous_watches` e `nexus_narrations` — colunas, índices e RLS conforme ADR 0019 §Decision
...
> `start_autonomous_mode`/`stop_autonomous_mode` (`web/lib/nexus/tools.ts`) inserem/encerram o
...
- `GET  /api/nexus/narrations?session=<id>` — narrações não faladas (≤1h, limit 10).
  200 `{ narrations: [{id,text,kind,image_path?,ts}] }`.
- `PATCH /api/nexus/narrations/:id` — marca `spoken_at` após o browser falar.

### 3.3 Skill `autonomous-watch-tick` (headless)

Input: `watch_id=<uuid>`. Lê o watch; ramifica por `phase` (`watching`→Passos 2–5, `reviewing`→
Passo R, `notifying`→Passo N; `done`/`failed`→no-op). Saídas: linhas em `nexus_narrations`,
...
- `scripts/screenshot-page.cjs --url <https> --watch <uuid> [--steps N]` — Playwright/Chromium;
  N prints por scroll; sobe ao bucket privado `nexus-review`; imprime
  `{ ok, shots: [{ storage_path, scroll_pct }], count }`. SSRF guard: só `https://*.example.com`.
...
- **Dois pedidos para o mesmo target** → índice único bloqueia o 2º; Nexus responde "já estou
  monitorando".
- **Screenshot falha** (página fora do ar) → narra o erro, pula revisão, ainda notifica.

## 5. Critérios de aceite

- [ ] `start_autonomous_mode` cria 1 watch e o Nexus fala a confirmação na aba correta.
...
- [ ] Email chega a `<OPERATOR_EMAIL>` com a URL.
- [ ] Watch encerra (`phase=done`) e o Nexus fala "saindo do modo autônomo".
...
- **Fase 1 — Fundação (sem voz/visão):** migrations (`autonomous_watches`, `nexus_narrations`,
  `claim_autonomous_watch`), endpoints start/stop/narrations, tools no Nexus, e o
...
- **Fase 2 — Revisão visual server-side ✅ (2026-06-04):** `scripts/screenshot-page.cjs`
  (Playwright/Chromium na imagem Fly, SSRF guard `*.example.com`), bucket privado `nexus-review`,
