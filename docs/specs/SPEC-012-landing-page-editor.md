# SPEC-012 — Landing pages editáveis em tempo real (Nexus + operador)

| Campo | Valor |
|---|---|
| Status | Draft → Implementing |
| Data | 2026-06-03 |
| Autor | cliente-exemplo (via Claude Code) |
...
- O **Nexus** edita por **voz**, perguntando qual seção/campo/modificação quando faltar
  parâmetro e **confirmando antes de aplicar**.
- A criação inicia pelo Nexus (pergunta cliente + produto) e direciona o operador para
...
|---|---|---|---|
| **Rascunho** | dashboard (`/lp-preview/[id]` em iframe) | Supabase (`landing_page_sections` + `theme` + `settings`) | instantânea (operador/Nexus) |
| **Publicado** | `<subdomain>.example.com` | snapshot estático no Cloudflare Pages | só no **Publicar** |
...
- **Rendering** compartilhado: as seções do template viram um pacote
  (`@template/lp-render`) consumido pelo template (export estático) **e** pelo web (preview
...
## 6. Contratos — tools do Nexus (`web/lib/nexus/tools.ts`)
...
- `get_landing_page(landing_page_id)` → seções (type/position) + chaves de campo + valores
  truncados + theme + settings (os "endereços" que o Nexus mira).
...
- `request_landing_page_section_image(landing_page_id, section_type, image_url, confirm)`
  ...
  vazio remove. O upload de arquivo é do operador (editor); o Nexus só aplica a URL. Mesmo
...
Skill headless `publish-landing-page-cliente-exemplo` (`.claude/skills/`), disparada pela
...
- **Concorrência operador × Nexus**: `version` otimista por seção; conflito → refetch +
...
- **Publish concorrente**: índice único per-LP rejeita 2º `landing_publish` → UI/Nexus
...
2. Rebuild do `curso-exemplo` pelo template refatorado gera `out/` equivalente ao baseline (Wave 1).
...
5. Nexus: "modifique o headline da hero da LP X" pergunta o que faltar, confirma, aplica;
   "publica a LP X" enfileira `landing_publish` (Wave 5 — ✅).
6. Publicar atualiza `<subdomain>.example.com` e grava `published_snapshot` (Wave 2/5).
...
O `web/` passou a consumir `@template/lp-render` (`file:` dep + `transpilePackages` — ADR 0017)
...
- **Isolamento de superfície (two root layouts).** O `app/` foi reorganizado em route groups:
  ...
  preview é autocontido com 1 import de `@template/lp-render/globals.css`.
...
### 9.3 Edição por voz — tools do Nexus (Wave 5)

`web/lib/nexus/tools.ts` ganhou 5 tools (mesmo padrão das de campanha: 2 turnos `confirm`,
...
`prompt.ts` (`NEXUS_SYSTEM_PROMPT`) ganhou o bloco "EDITAR E PUBLICAR LANDING PAGES" (descobrir→
...
- **Whitelist por tipo de seção** (`web/lib/landing/section-schemas.ts`): Zod `.strict()` por
  ...
  edição do Nexus. Os schemas foram validados contra os dados reais da fixture curso-exemplo-e2e (todas as
...
- **Auditoria** (`operation_logs`, entity_type=`landing_page`, action=`update`): publish (API
  actor=`operator`, Nexus actor=`nexus`) e edições autônomas do Nexus (texto/tema). Edições
...
- **Logo da marca** (page-level): `settings.logo` → `contentSpec.logo` → renderizada no topo do
  ...
  imagem por seção do Nexus (é page-level).
...
- **Nexus**: `request_landing_page_section_image` (§6). Validação Zod: `image: txt.optional()`
  nos schemas `.strict()`.
