...
Worker multi-tenant `track.example.com` (same-site, cookies first-party `.example.com`) →
...
do Worker (`https://track.example.com`); `lp_id` é o UUID da LP. Ausente ⇒ Fase 1 pura (sem POST
