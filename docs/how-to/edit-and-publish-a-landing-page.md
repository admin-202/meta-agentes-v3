...
> Receita prática (Diátaxis). Pressupõe uma LP já criada (pela skill de geração ou pelo Nexus)
...
4. Cada mudança reflete **na hora** no iframe e é salva sozinha (debounce ~600ms). Se aparecer
   um aviso de conflito, é porque o Nexus (ou outra aba) editou a mesma seção — o editor
...
## B. Editar pela voz (Nexus)

1. Peça ao Nexus para **listar**: "Nexus, quais landing pages tem o cliente cliente-exemplo?"
...
3. Edite um campo: "modifique o headline da hero da landing page X". Se faltar seção/campo/valor,
   o Nexus pergunta. Ele **relê o de/para e pede confirmação** — só aplica após o seu "sim".
   - Edições de **texto** e **tema** são baratas e **síncronas** (vão direto pro rascunho).
   - O Nexus edita **um campo escalar existente** por vez (texto/número/sim-não). Reordenar
     blocos, adicionar/remover itens de lista ou trocar imagem → use o painel.
4. Toda edição/publish do Nexus fica registrada em `operation_logs` (trilha de auditoria).
...
2. Pela voz: "Nexus, publica a landing page X" (ou "publica indexável" para go-live). Confirma
   em 2 turnos.
3. O publish **enfileira** um job `landing_publish`: o runner Fly serializa o rascunho →
   `next build` → `wrangler deploy` em `<subdomínio>.example.com`. Costuma levar ~1–2 min.
4. Acompanhe pelo dashboard (a LP fica `publishing` e volta a `ready` ao terminar) ou pergunte
   ao Nexus: "a publicação da landing page X terminou?".
...
- O preview é fiel ao publicado porque ambos usam o mesmo pacote de render (`@template/lp-render`).
