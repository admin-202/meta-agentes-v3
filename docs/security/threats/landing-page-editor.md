...
- **Tools de voz do Nexus** (`web/lib/nexus/tools.ts`): `list_landing_pages`,
...
- **Ameaça:** `fields` com chave hostil/desconhecida ou tipo errado (ex.: `comparison.rows`
  ...
  `validateSection(type, fields)` protege a API **e** as tools do Nexus.
...
- **Ameaça:** XSS por texto de copy. **Mitigação:** React escapa texto no render; **sem**
  `dangerouslySetInnerHTML` em nenhuma seção (`@template/lp-render`).
...
- **Ameaça:** edição/publish sem rastro. **Mitigação:** `operation_logs(entity_type='landing_page',
  ...
  do **Nexus** (actor=`nexus`) e todo **publish** (API actor=`operator`, Nexus actor=`nexus`).
...
- **Ameaça:** flood de publish drena build/deploy. **Mitigação:** rate limit `landingPublish`
  ...
  publish/edit ativo por LP) + confirmação em 2 turnos no Nexus + poller single-flight.
- **Ameaça:** upload gigante. **Mitigação:** `MAX_ASSET_BYTES` 5MB (413), MIME allowlist (415).

### E — Elevation of privilege
- **Ameaça:** Nexus dispara deploy/gasto indevido ou skill arbitrário. **Mitigação:** publish
...
- Edições interativas do operador na UI não são auditadas por-campo (só o publish e as ações do
  Nexus). Aceito: operador autenticado, presente, ação reversível até publicar.
...
- [ ] `validateSection(type, fields)` (estrutural + whitelist por tipo) em PATCH de seção **e** edit do Nexus
...
- [ ] `operation_logs` em publish (ambos) e edições do Nexus; logs sem PII
