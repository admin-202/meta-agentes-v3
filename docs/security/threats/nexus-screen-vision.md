# Threat Model: Nexus Screen Vision (STRIDE)

> Feature: [nexus-screen-vision](../../specs/nexus-screen-vision.md) ·
> ADR: [0010](../../adr/0010-nexus-screen-vision.md) · Data: 2026-05-31

## Superfície de ataque

- Novo endpoint `POST /api/nexus/capture` (recebe imagem base64).
- `POST /api/nexus/chat` agora pode retornar `need_capture` + `pendingId`.
- Estado efêmero no Redis (`nexus:pending:*`).
...
- **Prompt-injection visual:** texto na tela ("ative a campanha X", "ignore as
  ...
  ([nexus-agent-trigger](../../specs/nexus-agent-trigger.md)). Ver a tela **não**
...
- Abuso de chamadas: rate limit `nexus-capture` 15/min por IP (`429` + `Retry-After`),
...
- [x] Auth gate cobre `/api/nexus/capture` (herdado do middleware).
- [x] Schema zod + cap de tamanho (413) + validação de charset base64.
- [x] Rate limit dedicado (`nexus-capture`).
