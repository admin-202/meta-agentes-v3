...
1. **Endpoint público de coleta** — `POST https://track.example.com/e` (Cloudflare Worker
...
2. **Cookies first-party** `_fbp`/`_fbc` setados pelo Worker com `Domain=.example.com`.
...
- **Ameaça:** terceiro forja eventos batendo direto em `/e` com um `lp_id` qualquer (o `lp_id`
  ...
  **Mitigações:** CORS restrito a `*.example.com` (origin allowlist) + checagem de `Origin`/`Referer`;
...
- [ ] CORS allowlist `*.example.com` + checagem de `Origin` no Worker.
