...
- **Topologia:** um Worker multi-tenant em `track.example.com` (adaptado de `track_feature/`).
  Como toda LP é `*.example.com`, `track.example.com` é **same-site** (mesmo eTLD+1) → cookies
  `Domain=.example.com` são first-party e sobrevivem ao ITP; CORS restrito a `*.example.com`.
...
- Same-site sob `example.com` dá o benefício de cookie first-party do Stape **sem** Stape e sem
  Worker por LP.
...
- LP em domínio próprio de cliente (não `*.example.com`) perderia o same-site; fora de escopo
  hoje (todas as LPs são `*.example.com`). Solução futura: Pages Function same-origin por LP.
...
- **Worker (Pages Function) embutido por LP** (same-origin perfeito): funciona em domínio
  ...
  `*.example.com` — um Worker compartilhado same-site resolve com muito menos operação.
