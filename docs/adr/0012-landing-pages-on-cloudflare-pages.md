# ADR 0012 — Landing pages estáticas (Next.js export) no Cloudflare Pages sob `example.com`
...
| Relacionado | [ADR 0002](0002-supabase-meta-ads-persistence-schema.md) (convenções de persistência), [ADR 0003](0003-public-ad-ingest-bucket.md) (assets), [ADR 0009](0009-on-demand-agent-jobs-queue.md) (fila Nexus→Fly), [ADR 0005](0005-web-dashboard-on-vercel-monorepo.md) (Vercel) |

## Context

A agência hoje cria/ativa campanhas Meta Ads de forma headless (`claude -p "/skill"`),
disparadas pelo Nexus via fila `agent_jobs` → runner Fly. Falta o capability simétrico:
**produzir landing pages profissionais de alta conversão e publicá-las** sob subdomínios
de `example.com`, fracionando o processo (estrutura → copy → visual → tracking consent-gated →
deploy), também disparável de forma autônoma e pelo Nexus.

Restrições que moldam a decisão:

- **`example.com` é uma zona na mesma conta Cloudflare** onde os projetos Pages serão criados.
...
`out/` flat) e fazer deploy no Cloudflare Pages, um projeto por LP (`lp-<nome>`), com o
custom domain `<nome>.example.com`.** O template canônico vive em `landing-pages/_template/` e
é clonado para `landing-pages/<nome>/` por uma skill headless
(`create-landing-page-cliente-exemplo`), espelhando o contrato de `create-traffic-…`.
...
- **Template canônico** (`landing-pages/_template/`) — Next.js 15 export, i18n pt-BR via
  ...
  OG gerado, e tracking **consent-gated** (`Tracking.tsx` só injeta Pixel `<META_PIXEL_ID>`
  + GA4 `<GA4_MEASUREMENT_ID>` após `localStorage["lp_consent_v1"]`).
...
- **Deploy** — `wrangler pages project create lp-<nome>` + `wrangler pages deploy out` +
  bind do domínio (`POST .../pages/projects/lp-<nome>/domains`) + **criação explícita do
...
- **Disparo via Nexus** (Fase 2) — novo `kind='landing'` em `agent_jobs`, tool
...
- **Vercel project-por-LP** — rejeitado: acopla LPs efêmeras de cliente à infra do dashboard,
  gera sprawl de projetos Vercel, e o domínio `example.com` está na Cloudflare (precisaria de
