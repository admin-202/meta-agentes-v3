...
A skill `analytic-traffic-cliente-exemplo-campaign` rodava a cada 3 dias e era ancorada em
`OUTCOME_TRAFFIC`. Em 2026-06-10 uma rodada manual (analysis_id `6c05c907`) provou o framework
multi-objetivo na prática: a conta `<AD_ACCOUNT_ID>` tinha 6 campanhas ativas com gasto em 3
...
1. **Evoluir a skill existente** (mantendo o nome `analytic-traffic-cliente-exemplo-campaign`) para
...
- Consumidores (Nexus `get_latest_analysis`, daily-summary) seguem compatíveis: nenhum enum ou
