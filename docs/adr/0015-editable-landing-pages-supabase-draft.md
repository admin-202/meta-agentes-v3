...
operador quer **editar ao vivo** cada bloco/campo (manualmente e por voz via Nexus) com
...
- (b) **Supabase** como blocos editáveis, UI/Nexus escrevem direto; publish serializa.
...
- **Edição barata** (texto, cor, fonte): o web (Vercel) escreve **direto no Supabase**
  (síncrono) — tanto a UI quanto as tools do Nexus. Sem job, sem runner.
...
- ✅ Edição instantânea e multi-editor (operador + Nexus) sem tocar o site publicado.
