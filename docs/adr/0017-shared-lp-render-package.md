# ADR 0017 — Pacote compartilhado `@template/lp-render` (template + web) via `file:` dependency
...
- `packages/lp-render` é um pacote (`@template/lp-render`, `private`) com: tipos de conteúdo
...
- Cada app adiciona `"@template/lp-render": "file:../../packages/lp-render"` (caminho relativo
  ao app) e `transpilePackages: ["@template/lp-render"]` no `next.config`.
...
- ⚠️ As seções passam a ser **client components** (o `ContentProvider` usa context) —
  ...
  rebuild byte-a-byte do `curso-exemplo`.
...
- **Regressão zero confirmada.** Rebuild do `curso-exemplo` com o template refatorado → o
...
- **Subpath React-free preservado.** O barrel `.` agora reexporta também os client
  ...
  `@template/lp-render/serialize` (não pelo barrel) para não puxar JSX/React no Node.
...
A skill de publish (`publish-landing-page-cliente-exemplo`) roda o serializer **puro** no
...
- **Rodar com `tsx`, não `node` puro.** O type-stripping nativo do Node 22.18+ **não**
  ...
  `@template/lp-render/serialize` e a cadeia extensionless contra a fonte **inalterada**.
...
desligados, o webpack enxerga `@template/lp-render` pelo **caminho de symlink em
...
**excluir `@template/lp-render` dos `managedPaths`** do snapshot do webpack, para o pacote ser
rastreado por **conteúdo** e rebuildar a cada mudança de fonte:

```js
config.snapshot = {
  ...(config.snapshot ?? {}),
  managedPaths: [/^(.+?[\\/]node_modules[\\/](?!@template[\\/]lp-render))/],
};
```
