...
3. **Edição**: o endpoint `POST /api/landing-pages/:id/assets` (bucket `landing-assets`) já
   existia, mas sem UI no editor nem tool do Nexus para usar.
...
- **Logo da marca** (page-level, "completa"): `settings.logo` (DB) → `contentSpec.logo`
  ...
  no painel **Config** do editor (não é seção, então fora do tool de imagem por seção do Nexus;
...
- **Nexus**: tool `request_landing_page_section_image` seta/troca/remove a URL (mesmo fluxo de
...
- **Validação**: `image: txt.optional()` nos schemas Zod `.strict()` (write boundary); a tool do
  Nexus exige https + raster/landing-assets (bloqueia `javascript:`/`data:`).
