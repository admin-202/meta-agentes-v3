# ADR 0019 — Modo autônomo do Nexus (watch loop + narração + revisão visual)
...
| Data | 2026-06-04 |
| Decidido por | Equipe |
| Spec | [docs/specs/SPEC-013-nexus-autonomous-mode.md](../specs/SPEC-013-nexus-autonomous-mode.md) |
| Migrations | `20260604000001_add_autonomous_mode` + `20260604000002_add_nexus_review_bucket` (aplicadas) |
| Relacionado | [ADR 0001](0001-fly-machine-supercronic.md) (runner sem HTTP), [ADR 0007](0007-daily-summaries-and-agent-events.md) (polling deny-by-default + agent_events), [ADR 0009](0009-on-demand-agent-jobs-queue.md) (fila agent_jobs), [ADR 0010](0010-nexus-screen-vision.md) (screen vision), [ADR 0012](0012-landing-pages-on-cloudflare-pages.md) (URL publicada) |

## Context

O operador pediu um **"modo autônomo"** para o Nexus: depois de enfileirar uma tarefa longa
(ex.: gerar uma landing page, 5–25 min), ele quer poder **sair do computador** e que o Nexus:
...
4. **Notifique** `<OPERATOR_EMAIL>` por email que terminou;
5. **Encerre** o modo autônomo.

Restrições que moldam a decisão:

- O **Nexus headless** (skill em `claude -p` no runner Fly.io) é um **processo distinto** da
...
1. **Tabela `autonomous_watches`** (estado do modo autônomo). Colunas:
   ...
   `session_id text` (a aba do Nexus que deve falar) · `phase text`
...
2. **Tabela `nexus_narrations`** (canal servidor→browser, append-only). Colunas:
...
3. **Push da narração (reuso ADR 0007, sem Realtime):** o frontend do Nexus faz **polling**
   `GET /api/nexus/narrations?session=<id>&since=<ts>` (~2s, server-side com service key);
   para cada linha nova, **fala via TTS existente** (`/api/nexus/tts`, ADR 0011) e dá
   `PATCH` em `spoken_at`. Zero superfície de rede nova no runner; deny-by-default intacto.

4. **Tools tipadas no Nexus (web)** — `start_autonomous_mode(target_kind, target_id?)` e
...
5. **Skill `autonomous-watch-tick` (LLM, headless)** — disparada por tick. Por watch ativo:
   - **Fase `watching`**: lê `agent_jobs` (status) + `agent_events` (desde `last_event_ts`)
     do `run_id`, **resume os marcos novos em uma fala natural**, insere em `nexus_narrations`,
   ...
   - **Fase `reviewing`**: chama o **screenshotter server-side** (item 6) para `url`, obtém
     ...
     emite opinião** em `nexus_narrations` (`kind=opinion`, com `image_path`).
...
6. **Screenshotter server-side (Playwright na Fly)** — `scripts/screenshot-page.cjs --url <https>
   ...
   **`nexus-review`** (migration própria) e imprime o manifest JSON. **Independente da tela do
   operador** (atende o "vou ter que sair"). SSRF guard: só `https://*.example.com`.
...
- **`/loop` / `ScheduleWakeup` do harness Claude Code** — são do processo interativo, não do
  Nexus headless deployado. Não atravessam para a aba do operador. Rejeitado.
...
- **R**: cada narração e transição de fase fica em linha auditável (`nexus_narrations` +
  `operation_logs`).
