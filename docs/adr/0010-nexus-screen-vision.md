# 10. Nexus enxerga a tela do operador (screen vision)
...
O Nexus responde só com base nos dados do Supabase (tools server-side). Ele não
...
para então consultar o banco. Queremos dar ao Nexus a habilidade de **ver** a tela.
...
- **(A)** Screen Capture API (`getDisplayMedia`) com **stream persistente**: o
  operador clica uma vez em "Nexus pode ver minha tela" (gesto → seletor), escolhe
...
- **(B)** **Tool + resume**: `capture_screen` é uma tool *client-side* (sem handler
  ...
  no Redis (`nexus:pending:*`, TTL 120 s) e devolve `need_capture`. O cliente
  captura e chama `POST /api/nexus/capture`; o servidor retoma o loop injetando a
...
- A imagem pode conter dados de outras abas → tratado no threat model
  ([nexus-screen-vision](../security/threats/nexus-screen-vision.md)): consentimento,
...
- *Sempre anexa / heurística*: descartados — custo/latência por volta e fragilidade,
  e não modelam bem o "Nexus decide olhar" que o tool+resume captura.
